...
router.post('/master/task', requireAdmin, wrap(async (req, res) => {
  const { companyId, name, description, links, priority } = req.body || {};
  ...
  const out = await update((db) => {
    ...
    const mt = { id: uid('mt'), companyId, name: name.trim(), description: String(description || ''), links: cleanLinks(links), priority: cleanPriority(priority), active: true, order, subtasks: [] };
    db.master.push(mt);
...
router.put('/master/task/:id', requireAdmin, wrap(async (req, res) => {
  const { name, active, description, links, priority } = req.body || {};
  const out = await update((db) => {
    ...
    if (priority !== undefined) mt.priority = cleanPriority(priority);
    return mt;
  });
  res.json(out);
}));
  ...
  const out = await update((db) => {
    ...
    src.forEach((mt) => {
      ...
      db.master.push({
        ...
        description: mt.description || '', links: (mt.links || []).slice(), priority: cleanPriority(mt.priority),
        subtasks: mt.subtasks.map((s, i) => ({ id: uid('ms'), name: s.name, active: s.active, order: i, description: s.description || '', links: (s.links || []).slice() })),
      });
...
// Allot every sub-task of a task to the same user(s) in one go. Replaces the
// allottees on all of the task's sub-tasks; an optional dueDate, when supplied,
// is applied to all of them (blank leaves each sub-task's own due date untouched).
router.put('/tasks/:id/assign-all', requireAdmin, wrap(async (req, res) => {
  const { assignees, dueDate } = req.body || {};
  const list = Array.isArray(assignees) ? assignees : [];
  const out = await update((db) => {
    const t = db.tasks.find((x) => x.id === req.params.id);
    if (!t) throw httpError(404, 'Task not found');
    (t.subtasks || []).forEach((s) => {
      s.assignees = list.slice();
      if (dueDate) s.dueDate = dueDate;
    });
    return { count: (t.subtasks || []).length };
  });
  res.json(out);
}));

// Mark every sub-task of a task as completed (or back to not-completed). Admin
// affects all sub-tasks; a normal user only their own. `completed` defaults true.
router.put('/tasks/:id/complete-all', requireAuth, wrap(async (req, res) => {
  ...
  const out = await update((db) => {
    ...
    let count = 0;
    (t.subtasks || []).forEach((s) => {
      if (!isAdmin && !(s.assignees || []).includes(req.user.id)) return;
      s.status = completed ? 'completed' : 'pending';
      s.completedDate = completed ? new Date().toISOString() : null;
      s.completedBy = completed ? req.user.id : null;
      count++;
    });
    return { count };
  });
...
router.put('/subtasks/:id', requireAuth, wrap(async (req, res) => {
  ...
  const out = await update((db) => {
    ...
    if (body.status !== undefined) {
      if (!['pending', 'completed'].includes(body.status)) throw httpError(400, 'Invalid status');
  ...
  const out = await update((db) => {
    ...
    source.forEach((src) => {
      ...
      const task = {
        ...
        description: src.description || '', links: (src.links || []).slice(), priority: cleanPriority(src.priority),
        order: baseOrder++, createdAt: new Date().toISOString(),
...
// Remove every monthly task instance (keeps companies, master lists and users).
// Used to clear out tasks that the old code auto-propagated across all months, so
// each month starts empty until an admin imports/copies/adds into it. Admin only.
router.post('/maintenance/clear-tasks', requireAdmin, wrap(async (req, res) => {
