...
const { getDb, update, uid, MONTHS, financialYearList } = require('./db');
...
router.get('/assignments', requireAdmin, wrap(async (req, res) => {
  ...
  const users = db.users.map((u) => {
    const companies = {}; // companyId -> { id, name, active, tasks: { name -> { name, subtasks: [] } } }
    db.tasks.forEach((t) => {
      ...
      (t.subtasks || []).forEach((s) => {
        ...
        const task = c.tasks[t.name] || (c.tasks[t.name] = { name: t.name, priority: '', subtasks: [] });
...
function newSubInstance(s) {
  return {
    id: uid('st'), name: s.name, masterSubtaskId: s.id,
    description: s.description || '', links: (s.links || []).slice(),
    assignees: [], dueDate: null, status: 'pending', remarks: '', completedDate: null, completedBy: null,
  };
}

// Create an instance of a master task into every financial year × every month
// for its company (unassigned). Idempotent — skips months that already have it.
function propagateNewMasterTask(db, mt) {
  // every financial year in the visible window (2 back … 5 ahead), all 12 months
  const years = new Set([...financialYearList(), ...(db.meta.financialYears || [])]);
  years.forEach((fy) => {
    MONTHS.forEach((month) => {
      const has = db.tasks.some((t) => t.fy === fy && t.companyId === mt.companyId && t.month === month && t.masterTaskId === mt.id);
      if (has) return;
      const order = db.tasks.filter((t) => t.fy === fy && t.companyId === mt.companyId && t.month === month).length;
      db.tasks.push({
        id: uid('tsk'), fy, companyId: mt.companyId, month, name: mt.name, masterTaskId: mt.id,
        description: mt.description || '', links: (mt.links || []).slice(),
        order, createdAt: new Date().toISOString(),
        subtasks: (mt.subtasks || []).filter((s) => s.active).map(newSubInstance),
      });
    });
  });
}

// Add a newly-created master sub-task (unassigned) to every existing month
// instance of its parent master task. Idempotent.
function propagateNewMasterSubtask(db, masterTaskId, sub) {
  db.tasks.forEach((t) => {
    if (t.masterTaskId === masterTaskId && !t.subtasks.some((s) => s.masterSubtaskId === sub.id)) {
      t.subtasks.push(newSubInstance(sub));
    }
  });
}
...
router.post('/master/task', requireAdmin, wrap(async (req, res) => {
  ...
  const out = await update((db) => {
    ...
    db.master.push(mt);
    propagateNewMasterTask(db, mt); // auto-add to every month, unassigned
    return mt;
  });
  res.status(201).json(out);
}));
...
router.post('/master/task/:id/subtask', requireAdmin, wrap(async (req, res) => {
  ...
  const out = await update((db) => {
    ...
    propagateNewMasterSubtask(db, mt.id, sub); // auto-add to every month instance, unassigned
    return sub;
  });
  res.status(201).json(out);
}));
